...
export class DemoContainerComponent implements OnInit {
	...
	constructor(private router: Router, private demoService: DemoService, public ms: MenuService) {}
	...
	private setMetadata() {
		this.router.events
			.pipe(
				...
				flatMap((evt: NavigationEnd) => {
					let childroute = evt.url.substr(evt.url.lastIndexOf('/') + 1);
					return this.demos$.pipe(map((items) => items.find((i) => i.url.includes(childroute))));
				})
			)
			.subscribe((demo) => {
				this.header =
