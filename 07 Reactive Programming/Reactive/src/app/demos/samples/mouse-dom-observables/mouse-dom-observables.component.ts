...
export class MouseDomObservablesComponent {
	...
	private captureEvents() {
		...
		let mouse$ = fromEvent(canvasEl, 'mousedown').pipe(
			switchMap((e) => {
		...
		this.mouseSubscription = mouse$.subscribe((res: [MouseEvent, MouseEvent]) => {
	...
	private drawOnCanvas(prevPos: { x: number; y: number }, currentPos: { x: number; y: number }) {
		...
		//if (prevPos)  {
		if (Math.abs(prevPos.x - currentPos.x) > 0 || Math.abs(prevPos.y - currentPos.y) > 0) this.cx.beginPath();
	...
	subscribeSearchBox() {
		fromEvent(this.searchBox.nativeElement, 'keyup').subscribe((ke: KeyboardEvent) => {
